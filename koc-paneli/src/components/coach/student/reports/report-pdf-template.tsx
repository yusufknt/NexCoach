      ...
      >
        ...
        <div className="flex justify-between items-end border-b border-border pb-4">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-0.5">Kinetic Performance Coaching Portal</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 bg-muted/30 rounded-xl p-4 border border-border">
          <div className="text-sm">
            <span className="text-xs text-muted-foreground block">Öğrenci:</span>
            <span className="font-bold text-white text-base">Sporcu Gelişim Raporu</span>
          </div>
          <div className="text-sm text-right">
            <span className="text-xs text-muted-foreground block">Rapor Tarihi:</span>
          ...
          <div className="space-y-3">
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-muted/30">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-muted/30 text-primary uppercase tracking-wider text-[10px] border-b border-border">
                    <th className="py-2.5 px-4 font-bold border-r border-border">METRİK</th>
                    <th className="py-2.5 px-3 font-bold border-r border-border text-center">1. HAFTA</th>
                    <th className="py-2.5 px-3 font-bold border-r border-border text-center">2. HAFTA</th>
                    <th className="py-2.5 px-3 font-bold border-r border-border text-center">3. HAFTA</th>
                    <th className="py-2.5 px-3 font-bold border-r border-border text-center">4. HAFTA</th>
                ...
                <tbody className="divide-y divide-border text-foreground">
                  ...
                  <tr>
                    ...
                    {stats.weeklyBreakdown.map((w, idx) => {
                      ...
                      return (
                        <td key={idx} className="py-2 px-3 border-r border-border text-center">
                  ...
                  {/* Bel */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Ortalama Bel Ölçüsü</td>
                    {stats.weeklyBreakdown.map((w, idx) => {
                      const prev = idx > 0 ? stats.weeklyBreakdown[idx - 1] : null
                  ...
                  {/* Bench Press */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Bench Press Max</td>
                    {stats.weeklyBreakdown.map((w, idx) => {
                      const prev = idx > 0 ? stats.weeklyBreakdown[idx - 1] : null
                  ...
                  {/* Squat */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Squat Max</td>
                    {stats.weeklyBreakdown.map((w, idx) => {
                      const prev = idx > 0 ? stats.weeklyBreakdown[idx - 1] : null
                  ...
                  {/* Deadlift */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Deadlift Max</td>
                    {stats.weeklyBreakdown.map((w, idx) => {
                      const prev = idx > 0 ? stats.weeklyBreakdown[idx - 1] : null
                  ...
                  {/* Antrenman */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Antrenman Uyum Oranı</td>
                    {stats.weeklyBreakdown.map((w, idx) => (
                      <td key={idx} className="py-2 px-3 border-r border-border text-center">
                  ...
                  {/* Uyku */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Ortalama Uyku</td>
                    {stats.weeklyBreakdown.map((w, idx) => {
                      const prev = idx > 0 ? stats.weeklyBreakdown[idx - 1] : null
                  ...
                  {/* Adım */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Günlük Ortalama Adım</td>
                    {stats.weeklyBreakdown.map((w, idx) => {
                      const prev = idx > 0 ? stats.weeklyBreakdown[idx - 1] : null
                  ...
                  {/* Diyet */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Beslenme / Diyet Uyumu</td>
                    {stats.weeklyBreakdown.map((w, idx) => {
                      const prev = idx > 0 ? stats.weeklyBreakdown[idx - 1] : null
                  ...
                  {/* Enerji */}
                  <tr>
                    <td className="py-2 px-4 font-medium border-r border-border text-muted-foreground">Genel Enerji Seviyesi</td>
                    {stats.weeklyBreakdown.map((w, idx) => {
                      const prev = idx > 0 ? stats.weeklyBreakdown[idx - 1] : null
        ...
        <div className="space-y-3">
          ...
          <div className="bg-muted/30 rounded-xl p-5 border border-border min-h-[140px] text-sm leading-relaxed text-foreground whitespace-pre-wrap">
        ...
        {stats && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-4 gap-4">
              {stats.weeklyBreakdown.map((w, idx) => (
                <div key={idx} className="overflow-hidden rounded-xl border border-border bg-muted/30 flex flex-col justify-between h-[180px]">
                  <div className="relative flex-1 bg-black/40 flex items-center justify-center overflow-hidden">
                    {w.photo_url ? (
                      ...
                      <div className="text-center p-4">
                        <span className="text-[10px] text-muted-foreground block uppercase tracking-wider font-bold">Fotoğraf</span>
                        <span className="text-[8px] text-muted-foreground block uppercase tracking-wider mt-0.5">Yüklenmedi</span>
                      </div>
                    )}
                  </div>
                  <div className="p-2 text-center text-[10px] text-muted-foreground font-bold border-t border-border uppercase tracking-wider bg-muted/30">
                    {idx + 1}. HAFTA
                  </div>
                </div>
              ))}
        ...
        {/* Footer */}
        <div className="border-t border-border pt-6 flex justify-between items-center text-[10px] text-muted-foreground">
